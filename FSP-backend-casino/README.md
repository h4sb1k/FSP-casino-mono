...
Единый Java-сервис: Spring Boot backend + встроенные SPA (Opencase, Mountain, Bank) из `casino-app/src/main/resources/static`.

## О продукте

Платформа моделирует **единый бонусный кабинет**: у пользователя один баланс в баллах и зарезервированная сумма в активных комнатах. Игрок выбирает мини-игру с приветственного экрана или остаётся в **Opencase** — классическом лобби комнат с фильтрами по входу, слотам и VIP-тиру. В комнате баллы резервируются при входе; после старта раунда победитель считается **только на сервере** (взвешенный RNG с учётом буста), результат фиксируется в истории и рассылается по **STOMP**-топикам.

**Mountain** и **Bank** — отдельные фронтенд-модули (iframe с того же origin), используют общие `/api/auth/login` и кошелёк `/api/wallet/*`, чтобы баланс везде был согласован.

**Администрирование:** пользователь с ролью `ADMIN` (в сиде — `vip_player`) управляет глобальным конфигом и комнатами в статусе `WAITING` через `/api/admin/**` и веб-страницу **`/admin`**. Для админа в шелле и во встроенных играх предусмотрены переходы к этой панели.

**Таймер ожидания набора игроков** в Opencase на стороне сервера задаётся `waitingTimerSeconds` (конфиг админа и Flyway; целевое значение по умолчанию **60** секунд) и синхронизируется с Redis TTL и событиями таймера для клиентов.
...
| Сервис | URL |
|---|---|
| Приложение (frontend + backend) | http://localhost:8080 |
...
- SPA маршруты шелла Opencase: `/`, `/profile`, `/admin`; вложенные игры: `/mountain/`, `/bank/`, deep-link банковской локальной админки: `/bank/admin`
- Статические ассеты отдаются Spring Boot из `casino-app/src/main/resources/static`.
- Для обновления **Opencase** (корневой SPA в `static/`):
  1. Собрать фронт в `kazik-game/frontend-react` (`npm run build`)
  2. Скопировать **только** `dist/index.html` и каталог `dist/assets/` в `backend/casino-app/src/main/resources/static/` (не используйте `rsync --delete` из всего `dist/` в корень `static/`, иначе удалятся `mountain/`, `bank/`, `instruct_photos/`).
- Для **Mountain** и **Bank**: собрать в `external-games/...` и синхронизировать отдельно в `static/mountain/` и `static/bank/`.
